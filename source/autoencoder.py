...
LATENT_DIM = 16         # Size of the latent embedding bottleeck  & the song embedding size
...
class MusicAutoencoder(nn.Module):
    ...
    The encoder is trained with MSE loses on the input features, so it learns to compress the audio features
    Then during inference time, only the encoder is used to produce the 16-dimensional song embeddings, 
    which are compared with cosine similarity for recommendations.
    """
